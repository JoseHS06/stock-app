<script>
  ...
  $: badgeText =
    stock <= minimum
      ...
      : "text-success";
  $: badge =
    stock <= minimum ? (stock == 0 ? "bg-danger" : "bg-warning") : "bg-success";
  $: badgeContent =
    stock <= minimum
      ? stock == 0
        ? "Sin stock"
  ...
  const deleteProduct = async () => {
    const response = await deleteItem(1, id);
...
<div class="col-lg-4 col-sm-12 mt-4">
  ...
  <div
    ...
    class="card"
    style="cursor: pointer; background-color: #13161b"
  >
    <div
    ...
    >
      ...
      <p class="card-text text-muted text-white">
        <span class={badgeText}>{badgeContent}</span>
      </p>
      <span
        class="position-absolute top-0 start-100 translate-middle badge rounded-pill {badge}"
      >
        {stock}
      </span>
    </div>
